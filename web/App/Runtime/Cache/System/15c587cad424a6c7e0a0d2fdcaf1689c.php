            ...
            <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6 text-xs-center">
                            <div class="form-group">
                                <a href="<?php echo U('adds');?>"><button id="demo-btn-addrow" class="btn btn-purple"><i class="demo-pli-plus"></i>添加通道</button></a>
                            </div>
                        </div>
                ...
                <table id="demo-foo-addrow" class="table table-bordered table-hover toggle-circle" data-page-size="7">
                    ...
                    <tr>
                        ...
                        <th class="text-center">能否变更</th>
                        <th class="text-center">创建时间</th>
                        <th class="text-center">通道描述</th>
                        <th class="text-center">通道备注</th>
                        <th class="text-center">是否维护</th>
                    ...
                    <?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
                            ...
                            <td class="text-center"><span class="label label-mint" style="background-color:#<?php echo ((isset($v["color"]) && ($v["color"] !== ""))?($v["color"]):'26a69a'); ?>;font-size:12px;font-weight:1;border-radius: 5px;"><?php echo ($v['type']); ?></span></td>
                            <td class="text-center">
                                <?php if($v['status'] == 1): ?><span class="label label-danger" style="font-size:12px;font-weight:1;border-radius: 5px;">已开启</span>
                                    <?php else: ?>
                                    <span class="label label-default" style="font-size:12px;font-weight:1;border-radius: 5px;">未开启</span><?php endif; ?>
                            </td>
                            <td class="text-center">
                                <?php if($v['alter_status'] == 1): ?><span class="label label-danger" style="font-size:12px;font-weight:1;border-radius: 5px;">可以变更</span>
                                    <?php else: ?>
                                    <span class="label label-default" style="font-size:12px;font-weight:1;border-radius: 5px;">不能变更</span><?php endif; ?>
                            </td>
                            <td class="text-center"><?php echo (date('Y-m-d H:i:s',$v['createtime'])); ?></td>

                            <td class="text-center"><span class="label label-default" onclick="data_dialog('<?php echo ($v['name']); ?>','<?php echo ($v['data']); ?>');" style="cursor:pointer;font-size:12px;font-weight:1;border-radius: 5px;">点击查看</span></td>
                            ...
                            <td class="text-center">
                                ...
                                <?php if(($v["stop_status"]) == "1"): ?>checked=""<?php endif; ?>
                                type="checkbox" name="stop_status">
                                <label id="status_switch" for="status-switch-<?php echo ($v['id']); ?>"></label>
                            </td>
                            <td class="text-center">
                                <a href="<?php echo U('edits',array('id'=>$v['id']));?>" class="btn btn-mint btn-rounded btn-sm">编辑</a>
                            </td>
...
<script>
    function data_dialog(name,data) {
            bootbox.dialog({
                title: "通道数据详情",
                message: '<div class="media"><div class="media-body"><p class="text-semibold text-main">通道描述:'+name+'</p>'+data+'</div></div>',
                buttons: {
                    confirm: {
                        label: "取消"
                    }
    ...
    $(".status_switch").change(function () {
        ...
        var ajax_data = {id: this.value, status: status, type: 'stop_status'};
        var actionurl = "<?php echo U('stop_status');?>";
