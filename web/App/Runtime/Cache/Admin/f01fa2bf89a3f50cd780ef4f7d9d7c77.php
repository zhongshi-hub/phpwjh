...
<div id="content-container">
    <div id="page-content">
        <div class="panel">
            ...
            <form method="post" action="<?php echo U('adds');?>" ajax="yes">
                    ...
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <select data-placeholder="请选择..." tabindex="2" class="form-control" name="type" required="required">
                                    <option value="wx" <?php if(($data["type"]) != "ali"): ?>selected<?php endif; ?> >微信刷脸设备</option>
                                    <option value="ali" <?php if(($data["type"]) == "ali"): ?>selected<?php endif; ?> >支付宝刷脸设备</option>
                                </select>
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <select data-placeholder="请选择..." tabindex="2" class="form-control" name="status" required="required">
                                    <option value="0" <?php if(($data["status"]) == "0"): ?>selected<?php endif; ?> >禁用</option>
                                    <option value="1" <?php if(($data["status"]) != "0"): ?>selected<?php endif; ?> >启用</option>
                                </select>
                                <span>如设置禁用 设备将不可用</span>
                            </div>
                        </div>
                    </div>
                    <p style="border-bottom: solid 1px #E4E4E4;font-size: 16px;padding-bottom: 5px;">设备信息</p>
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label class="control-label">设备型号</label>
                                <input type="text" name="device_number" class="form-control" value="<?php echo ($data["device_number"]); ?>" placeholder="" required>
                            </div>
                        </div>
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label class="control-label">设备序列号(SN)</label>
                                <input type="text" name="device_sn" class="form-control" value="<?php echo ($data["device_sn"]); ?>" placeholder="" required>
                            </div>
                        </div>
                    </div>

                    <p style="border-bottom: solid 1px #E4E4E4;font-size: 16px;padding-bottom: 5px;">商户信息(可选)</p>
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label class="control-label">所属商户</label>
                                <div class="input-group mar-btm">
                                <input id="dialog_mch_name" class="form-control mid_dialog" type="text"  value="<?php $seller=Get_Seller($data['pay_mid']); echo $seller['mch_name']; ?>">
                                <input id="dialog_mid" class="form-control" type="hidden" name="mid" value="<?php echo ($data["mid"]); ?>">
                                <span class="input-group-btn">
                                    <button class="btn btn-default mch_rest" type="button" title="清空选择" style="border-radius: 0px !important;"><i class="fa fa-close"></i></button>
                                </span>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <input id="dialog_store_id" class="form-control" type="hidden" name="store_id" value="">
                                    <span class="input-group-btn">
                                    <button class="btn btn-default store_rest" type="button" title="清空选择" style="border-radius: 0px !important;"><i class="fa fa-close"></i></button>
                                </span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <p style="font-size: 14px;">选择所属商户设备可在当前商户下所有门店使用,选择所属门店设备只能当前门店下使用</p>
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group pad-ver">
                                <label class="control-label">设备备注</label>
                                <textarea class="form-control" name="info" style="height: 100px;resize: none"></textarea>
                            </div>
                        </div>
                    </div>
                    <p style="font-size: 14px;">广告费用设置</p>
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label class="control-label">按年收费</label>
                                <input type="text" name="ad_year" class="form-control"  placeholder="单位:元">
                            </div>
                        </div>
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label class="control-label">按月收费</label>
                                <input type="text" name="ad_month" class="form-control" placeholder="单位:元" >
                            </div>
                        </div>
                    </div>

                    <div class="alert alert-danger">
                        <p>首次商户在设备登录自动绑定信息</p> <p>如更换商户 请使用设备列表页清除绑定功能</p>
                    </div>
...
<script>
    ...
    $('.mch_rest').click(function () {
        $('#dialog_mch_name,#dialog_mid').val('');
    });
    $('.store_rest').click(function () {
        $('#dialog_store_name,#dialog_store_id').val('');
    });
