...
<div id="content-container">
    <div id="page-content">
        <div class="panel">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <form method="post" action="<?php echo U('alter_log');?>" ajax="n">
            ...
            <div class="panel-body">
                <table  class="table table-bordered table-hover toggle-circle table-vcenter" data-page-size="7">
                    ...
                    <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
                            <td class="text-center"><?php $seller=Get_Seller($v['cid']); echo $seller['mch_name']; ?></td>
                            <td class="text-center">
                                <?php if($v['alleys'] == 'Wlbpays'): ?>(CT1|CD0)通道
                                    <?php else: ?>
                                    <?php echo (alleys_name($v["alleys"])); endif; ?>
                            </td>
                            <td class="text-center">
                                <?php if($v['type'] == 'rate'): ?><span class="label label-danger" style="font-size:12px;font-weight:1;border-radius: 5px;">费率信息变更</span>
                                    <?php else: ?>
                                    <span class="label label-mint" style="font-size:12px;font-weight:1;border-radius: 5px;">结算信息变更</span><?php endif; ?>
                            </td>
                            <td class="text-center"><?php echo (date('Y-m-d H:i:s',$v['time'])); ?></td>
                            <td class="text-center">
                                <?php if($v['status'] == 1): ?><span class="label label-danger" style="font-size:12px;font-weight:1;border-radius: 5px;">变更成功</span>
                                    <?php else: ?>
                                    <span class="label label-default" style="font-size:12px;font-weight:1;border-radius: 5px;">变更失败</span><?php endif; ?>
                            </td>
                            <td class="text-center">
                                <span class="label label-info" onclick="detail('<?php echo ($v['type']); ?>',<?php echo ($v["id"]); ?>)" style="cursor:pointer;font-size:12px;font-weight:1;border-radius: 5px;">点击查看</span>
                            </td>
                            <td class="text-center">
                                <?php echo sys_op_id($v['opid'],$v['system']); ?>
                            </td>
    ...
    function detail(type,id) {
        if(type=='rate') {
            layer.open({
                ...
                fixed: false, //不固定
                maxmin: false,
                content: "<?php echo U('alter_log_data');?>/type/" + type + "/id/" + id + "?Debug=1"
            });
        }else{
            layer.open({
                type: 2,
