    ...
    <div id="page-content">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title"><?php echo ((isset($title) && ($title !== ""))?($title):'口碑开店'); ?></h3>
            </div>
            <!--筛选功能开始-->
            <div class="row">
                <div class="col-sm-12">
                    <form method="post" action="<?php echo U('shopList');?>" ajax="n">
                                ...
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        ...
                                        <select  class="form-control" name="status">
                                            ...
                                            <option value="INIT" style="color: #99a79e">初始</option>
                                            <option value="PROCESS" style="color: #1d8be9">处理中</option>
                                            <option value="SUCCESS" style="color: #8651ff">成功</option>
                                            <option value="FAIL" style="color: #de2f54">失败</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        <label class="control-label">申请子状态</label>
                                        <select  class="form-control" name="audit_status">
                                            <option value="">所有状态</option>
                                            <option value="AUDITING" style="color: #99a79e">审核中</option>
                                            <option value="WAIT_CERTIFY" style="color: #b7bf3c">等待认证</option>
                                            <option value="LICENSE_AUDITING" style="color: #8651ff">证照审核中</option>
                                            <option value="RISK_AUDITING" style="color: #1d8be9">风控审核中</option>
                                            <option value="WAIT_SIGN" style="color: #03ab99">等待签约</option>
                                            <option value="FINISH" style="color: #06c64c">终结</option>
                                        </select>
                                    </div>
                                </div>
                                <div>
            ...
            <div class="panel-body">
                <div class="pad-btm form-inline">
                    <div class="row">
                        <div class="col-sm-6 text-xs-center">
                            <div class="form-group">
                                <button id="mchIn" class="btn btn-purple"><i class="demo-pli-add icon-fw"></i>口碑入网</button>
                            </div>
                        </div>
                ...
                <table  class="table table-bordered table-hover toggle-circle table-vcenter" data-page-size="7">
                    <thead>
                    <tr>
                        <th class="text-center">支付宝流水ID</th>
                        <th class="text-center" style="width: 100px">关联商户</th>
                        <th class="text-center">服务商PID</th>
                        <th class="text-center">门店ID</th>
                        <th class="text-center" style="width: 100px">主门店名称</th>
                        <th class="text-center">申请时间</th>
                        <th class="text-center">更新时间</th>
                        <th class="text-center">申请主状态</th>
                        <th class="text-center">申请子状态</th>
                        <!--<th class="text-center">操作</th>-->
                    </tr>
                    </thead>
                    <tbody>
                    <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
                            <td class="text-center" title="<?php echo ($v["request_id"]); ?>">
                               <?php echo ($v["apply_id"]); ?>
                            </td>
                            ...
                            <td class="text-center">
                                <?php echo ((isset($v["shop_id"]) && ($v["shop_id"] !== ""))?($v["shop_id"]):"---"); ?>
                            </td>
                            <td class="text-center">
                                <?php echo ($v["main_shop_name"]); ?>
                            </td>
                            ...
                            <td class="text-center">
                                <?php switch($v["status"]): case "INIT": ?><span class="state label label-danger" data-id="<?php echo ($v["id"]); ?>" style="background-color:#99a79e;font-size:12px;font-weight:1;border-radius: 5px;">初始</span><?php break;?>
                                    <?php case "PROCESS": ?><span class="state label label-danger"  data-id="<?php echo ($v["id"]); ?>" style="background-color:#1d8be9;font-size:12px;font-weight:1;border-radius: 5px;">处理中</span><?php break;?>
                                    <?php case "SUCCESS": ?><span class="state label label-danger"  data-id="<?php echo ($v["id"]); ?>" style="background-color:#8651ff;font-size:12px;font-weight:1;border-radius: 5px;">成功</span><?php break;?>
                                    <?php case "FAIL": ?><span class="state label label-danger"  data-id="<?php echo ($v["id"]); ?>" style="background-color:#de2f54;font-size:12px;font-weight:1;border-radius: 5px;">失败</span><?php break;?>
                                    <?php default: ?>
                                    <span class="state label label-default"  data-id="<?php echo ($v["id"]); ?>" style="font-size:12px;font-weight:1;border-radius: 5px;"><?php echo ((isset($v["status"]) && ($v["status"] !== ""))?($v["status"]):"待处理"); ?></span><?php endswitch;?>
                            </td>
                            <td class="text-center">
                                <?php switch($v["audit_status"]): case "AUDITING": ?><span class="state label label-danger" data-id="<?php echo ($v["id"]); ?>" style="background-color:#99a79e;font-size:12px;font-weight:1;border-radius: 5px;">审核中</span><?php break;?>
                                    <?php case "WAIT_CERTIFY": ?><span class="state label label-danger"  data-id="<?php echo ($v["id"]); ?>" style="background-color:#b7bf3c;font-size:12px;font-weight:1;border-radius: 5px;">等待认证</span><?php break;?>
                                    <?php case "LICENSE_AUDITING": ?><span class="state label label-danger"  data-id="<?php echo ($v["id"]); ?>" style="background-color:#8651ff;font-size:12px;font-weight:1;border-radius: 5px;">证照审核中</span><?php break;?>
                                    <?php case "RISK_AUDITING": ?><span class="state label label-danger"  data-id="<?php echo ($v["id"]); ?>" style="background-color:#1d8be9;font-size:12px;font-weight:1;border-radius: 5px;">风控审核中</span><?php break;?>
                                    <?php case "WAIT_SIGN": ?><span class="state label label-danger"  data-id="<?php echo ($v["id"]); ?>" style="background-color:#03ab99;font-size:12px;font-weight:1;border-radius: 5px;">等待签约</span><?php break;?>
                                    <?php case "FINISH": ?><span class="state label label-danger"  data-id="<?php echo ($v["id"]); ?>" style="background-color:#06c64c;font-size:12px;font-weight:1;border-radius: 5px;">终结</span><?php break;?>
                                    <?php default: ?>
                           ...
                           <!-- <td class="text-center">
                                <a href="<?php echo U('info',['apply_id'=>$v['apply_id'],'m'=>$v['mid']]);?>" class="btn btn-sm btn-default btn-active-primary">
                                    资料
                                </a>
                            </td>-->
...
<script>
    ...
    $('#mchIn').click(function () {
        ...
        if(mid==''||!mid){
            ...
            window.location.href = "<?php echo U('Isv/shopCreate');?>/id/" + mid;
        }
    });
