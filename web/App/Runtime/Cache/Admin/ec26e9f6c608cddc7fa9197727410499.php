    ...
    <div id="page-content">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title"><?php echo ((isset($title) && ($title !== ""))?($title):'商户升级'); ?></h3>
            </div>
            ...
            <div class="panel-body">
                <table  class="table table-bordered table-hover toggle-circle table-vcenter" data-page-size="7">
                    ...
                    <tr>
                        ...
                        <th class="text-center">子商户号</th>
                        <th class="text-center" style="width: 300px">升级商户名称</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center"><?php echo (date('Y-m-d H:i:s',$create_time)); ?></td>
                            <td class="text-center"><?php echo (date('Y-m-d H:i:s',$update_time)); ?></td>
                            <td class="text-center">
                                <?php switch($applyment_state): case "AUDITING": ?><span class="state label label-danger" data-id="<?php echo ($id); ?>" style="background-color:#99a79e;font-size:12px;font-weight:1;border-radius: 5px;">审核中</span><?php break;?>
                                    <?php case "REJECTED": ?><span class="state label label-danger"  data-id="<?php echo ($id); ?>" style="background-color:#de2f54;font-size:12px;font-weight:1;border-radius: 5px;">已驳回</span><?php break;?>
                                    <?php case "FROZEN": ?><span class="state label label-danger"  data-id="<?php echo ($id); ?>" style="background-color:#8651ff;font-size:12px;font-weight:1;border-radius: 5px;">已冻结</span><?php break;?>
                                    <?php case "NEED_SIGN": ?><span class="state label label-danger"  data-id="<?php echo ($id); ?>" style="background-color:#1d8be9;font-size:12px;font-weight:1;border-radius: 5px;">待签约</span><?php break;?>
                                    <?php case "FINISH": ?><span class="state label label-danger"  data-id="<?php echo ($id); ?>" style="background-color:#06c64c;font-size:12px;font-weight:1;border-radius: 5px;">已完成</span><?php break;?>
                                    <?php case "CHECKING": ?><span class="state label label-danger"  data-id="<?php echo ($id); ?>" style="background-color:#ffb42c;font-size:12px;font-weight:1;border-radius: 5px;">资料校验中</span><?php break;?>
                                    <?php case "ACCOUNT_NEED_VERIFY": ?><span class="state label label-danger"  data-id="<?php echo ($id); ?>" style="background-color:#2fc695;font-size:12px;font-weight:1;border-radius: 5px;">待账户验证</span><?php break;?>
                                    <?php default: ?>
                                    <span class="state label label-default"  data-id="<?php echo ($id); ?>" style="font-size:12px;font-weight:1;border-radius: 5px;">未知状态</span><?php endswitch;?>
                            </td>
                            <td class="text-center">
                                <button onclick="window.location.href='<?php echo U('upInfoCreate',['id'=>$_GET['id'],'business_code'=>$_GET['business_code']]);?>'" class="btn btn-sm btn-default btn-active-primary" type="button" aria-expanded="false" <?php if(($applyment_state) != "REJECTED"): ?>disabled<?php endif; ?>>
                                重新入网
                                </button>
                            </td>
                        </tr>
...
<script>
    $('.state').click(function (e) {
        var id=$(this).data('id');
        loading('请求审核结果中...');
        $.post('<?php echo U("applyUpState");?>', {id:id}, function (data) {
            if (data.status) {
                layer.closeAll();
