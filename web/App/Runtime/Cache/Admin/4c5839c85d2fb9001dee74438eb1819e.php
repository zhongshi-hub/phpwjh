...
<div id="content-container">
    <div id="page-content">
        <div class="panel">
            ...
            <!--<div class="row">
                <div class="col-sm-12">
                    <form method="post" action="<?php echo U('_log');?>" ajax="n">
                        <div class="panel-body">
                            <div class="row ">
                                ...
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        <label class="control-label">变更结果</label>
                                        <select  class="form-control" name="status" >
                                            <option value="">所有状态</option>
                                            <option value="1">变更成功</option>
                                            <option value="0">变更失败</option>
                                        </select>
                                    </div>
                                </div>
                                <div>
            ...
            <div class="panel-body">
                <div class="row text-center">
                    <div class="col-lg-12">
                        <div class="panel">
                            <div class="panel-body">
                                <div class="col-xs-4" >
                                    <p class="h2  text-thin mar-no pad-all"><?php echo ((isset($va1) && ($va1 !== ""))?($va1):"0"); ?></p>
                                    <small class="text-lg">总鉴权次数</small>
                                </div>
                                <div class="col-xs-4">
                                    <p class="h2 text-thin mar-no pad-all"><?php echo ((isset($va2) && ($va2 !== ""))?($va2):"0"); ?></p>
                                    <small class="text-lg">已用鉴权次数</small>
                                </div>
                                <div class="col-xs-4" >
                                    <p class="h2  text-thin mar-no pad-all"><?php echo ((isset($va3) && ($va3 !== ""))?($va3):"0"); ?></p>
                                    <small class="text-lg">剩余鉴权次数</small>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <table  class="table table-bordered table-hover toggle-circle table-vcenter" data-page-size="7">
                    ...
                    <tr>
                        ...
                        <th class="text-center">鉴权类型</th>
                        <th class="text-center">鉴权时间</th>
                        <th class="text-center">鉴权结果</th>
                        <th class="text-center">鉴权数据</th>
                        <th class="text-center">操作者</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
                            <td class="text-center"><?php $seller=Get_Seller($v['mid']); if($seller['mch_name']){echo $seller['mch_name'];}else{ echo $v['mid'];}; ?></td>
                            <td class="text-center">
                                <?php if(($v["alleys"]) == "mch_add_card"): ?><span style="color: #b6b6b6">MchAll</span>
                                <?php else: ?>
                                <?php echo (alleys_name($v["alleys"])); endif; ?>
                            </td>
                            <td class="text-center">
                                <?php if($v['source'] == '变更'): ?><span class="label label-danger" style="font-size:12px;font-weight:1;border-radius: 5px;">信息变更</span>
                                    <?php elseif($v['source'] == '进件'): ?>
                                    <span class="label label-mint" style="font-size:12px;font-weight:1;border-radius: 5px;">商户进件</span>
                                    <?php else: ?>
                                    <span class="label label-success" style="font-size:12px;font-weight:1;border-radius: 5px;"><?php echo ($v["source"]); ?></span><?php endif; ?>
                            </td>
                            <td class="text-center"><?php echo ($v['time']); ?></td>
                            <td class="text-center">
                                <?php if($v['status'] == 1): ?><span class="label label-danger" style="font-size:12px;font-weight:1;border-radius: 5px;">鉴权成功</span>
                                    <?php else: ?>
                                    <span class="label label-default" style="font-size:12px;font-weight:1;border-radius: 5px;">鉴权失败</span><?php endif; ?>
                            </td>

                            <td class="text-center">
                                <button onclick="v_swing(<?php echo ($v["id"]); ?>)" class="btn btn-info btn-rounded btn-sm">详细信息</button>
                                <input type="hidden" id="<?php echo ($v["id"]); ?>_card" value='<?php echo ($v["card"]); ?>'>
                                <input type="hidden" id="<?php echo ($v["id"]); ?>_cert" value='<?php echo ($v["cert"]); ?>'>
                                <input type="hidden" id="<?php echo ($v["id"]); ?>_name" value='<?php echo ($v["name"]); ?>'>
                                <input type="hidden" id="<?php echo ($v["id"]); ?>_phone" value='<?php echo ($v["phone"]); ?>'>
                                <?php if($v['status'] == 1): ?><input type="hidden" id="<?php echo ($v["id"]); ?>_validate" value='鉴权成功'>
                                    <?php else: ?>
                                    <input type="hidden" id="<?php echo ($v["id"]); ?>_validate" value='<?php echo ($v["msg"]); ?>'><?php endif; ?>
                            </td>
                            <td class="text-center">
                                <?php $res=sys_op_id($v['op_id'],$v['sys']); if($res){echo $res;}else{echo '商户';} ?>
                            </td>
...
<script src="/Source/statics/plugins/bootbox/bootbox.min.js"></script>
<script>
    function v_swing(id) {
        var card=$('#'+id+'_card').val();
        var cert=$('#'+id+'_cert').val();
        var name=$('#'+id+'_name').val();
        var phone=$('#'+id+'_phone').val();
        var validate=$('#'+id+'_validate').val();
        if(phone) {
            bootbox.alert({
                message: "<p class='text-semibold text-main'>鉴权数据详细信息</p><p>姓名</p><p class='well well-sm'>"+name+"</p><p>身份证号</p><p class='well well-sm'>"+cert+"</p><p>银行卡号</p><p class='well well-sm'>"+card+"</p><p>手机号</p><p class='well well-sm'>"+phone+"</p><p>鉴权返回结果提示</p><p class='well well-sm'>"+validate+"</p>",
                animateIn: 'rubberBand',
                animateOut: 'wobble'
            });
        }else{
            bootbox.alert({
                message: "<p class='text-semibold text-main'>鉴权数据详细信息</p><p>姓名</p><p class='well well-sm'>"+name+"</p><p>身份证号</p><p class='well well-sm'>"+cert+"</p><p>银行卡号</p><p class='well well-sm'>"+card+"</p><p>鉴权返回结果提示</p><p class='well well-sm'>"+validate+"</p>",
                animateIn: 'rubberBand',
                animateOut: 'wobble'
            });
        }
