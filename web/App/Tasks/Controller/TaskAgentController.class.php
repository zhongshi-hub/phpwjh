...
#数据统计
class TaskAgentController extends Controller
{
    ...
    public function statistics(){
        $where['status']=1;
        $AgentData=M('MchAgent')->where($where)->select();
        foreach ($AgentData as $Aval){
            $map['status']=1;
            $map['agent_id']=array('in',self::AgentAll($Aval['id']));
            $map['domain_auth']=$Aval['domain_auth'];
            $db=M('MchOrders');
            #数据汇总
            #总交易额
            ...
            $sys_data=array(
                ...
                'domain_auth'=>$Aval['domain_auth'],
                'agent'=>$Aval['id'],
            );
			$cache_agent='agent_'.$Aval['domain_auth'].$Aval['id'];
			S($cache_agent,$sys_data);
            //rwlog('agent_data',$sys_data);
//            $result=$Static->where(array('type'=>'agent','domain_auth'=>$Aval['domain_auth'],'agent'=>$Aval['id']))->count();
//            if($result){
//                $Static->where(array('type'=>'agent','domain_auth'=>$Aval['domain_auth'],'agent'=>$Aval['id']))->save($sys_data);
//            }else{
//                $Static->add($sys_data);
//            }
        }
