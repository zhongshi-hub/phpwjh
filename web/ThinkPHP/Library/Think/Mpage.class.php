...
class Mpage {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'<ul class="mui-pagination">  %upPage%  %first%  %prePage%  %linkPage%  %nextPage% %end% %downPage% </ul>');
    // 默认分页变量名
    protected $varPage;
    ...
    public function __construct($totalRows,$listRows='',$parameter='',$url='') {
        ...
        $this->varPage      =   C('VAR_PAGE') ? C('VAR_PAGE') : 'pages' ;
        if(!empty($listRows)) {
        ...
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                ...
                if($this->totalPages != 1){
                    $linkPage .= '<li class="mui-active"><a href="javascript:;">'.$page."</a></li>";
                }
            }
        }
        $pageStr     =   str_replace(
